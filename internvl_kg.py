...
KG_FILE = "/mnt/disk11/user/xiaoyih1/xh/krr/krr-project/knowledge_graph/kg_concepts.json"   # 改成你的真实路径
IMAGE_DIR = "/mnt/disk11/user/xiaoyih1/xh/krr/image"

OUTPUT_FILE = "/mnt/disk11/user/xiaoyih1/xh/krr/krr-project/results/predictions_internvl_kg_concepts_50.jsonl"
RESULT_FILE = "/mnt/disk11/user/xiaoyih1/xh/krr/krr-project/results/results_internvl_kg_concepts_50.json"
...
def get_surface_texts_for_idx(kg_data, idx, max_triples=None):
    """
    从 kg_data 中取出某个 idx 对应 triples 的 surfaceText
    先不过滤，只做简单清理
    """
    item = kg_data.get(str(idx), None)
    if item is None:
        item = kg_data.get(idx, None)
    ...
    for t in triples:
        s = clean_surface_text(t.get("surfaceText", ""))
        if s:
            surface_texts.append(s)

    if max_triples is not None:
        surface_texts = surface_texts[:max_triples]

    return surface_texts


def build_kg_text(surface_texts):
    if not surface_texts:
        return ""
    return "\n".join(f"- {s}" for s in surface_texts)
...
for sample in questions:
    ...
    # ===== KG =====
    surface_texts = get_surface_texts_for_idx(kg_data, idx, max_triples=None)
    kg_text = build_kg_text(surface_texts)
...
Answer with only one letter: A, B, C, or D."""
    ...
    print(f"idx {idx} | pred {pred} | gt {gt} | correct {is_correct} | n_kg={len(surface_texts)} | raw={repr(output_text)}")
